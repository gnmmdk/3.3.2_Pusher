...
void AudioChannel::initAudioEncoder(int sample_rate, int channels) {
    ...
    //todo B.1.2 第二步：配置编码器参数
    faacEncConfigurationPtr config = faacEncGetCurrentConfiguration(audioEncoder);
    //todo B.1.2.1 mpeg4标准
    config->mpegVersion = MPEG4;
    //todo B.1.2.2 LC标准
    config->aacObjectType = LOW;
    //todo B.1.2.3 16位
    config->inputFormat = FAAC_INPUT_16BIT;
    //todo B.1.2.4 编码出来的是原始数据，不是ADTS
    config->outputFormat = 0 ;
    //todo B.1.2.5 噪声控制（降噪）
    config->useTns = 1;
    config->useLfe = 0;//  LFE channel  低音炮？ =0 设置为无
    int ret = faacEncSetConfiguration(audioEncoder,config);
    if(!ret){
        LOGE("音频编码器参数配置失败");
        return;
    }
    //todo B.1.3 实例化 输出缓冲区
